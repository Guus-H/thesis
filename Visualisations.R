...
#==== Basic variables ====
fit <- lm(radicality ~ reg_div + continent + applicant_type ,data = ApplnStats, na.action = na.omit)
summary(fit)
...
==== Measuring skewness ====
  skewness(log(na.omit(ApplnStats$radicality)))
kurtosis(log(na.omit(ApplnStats$radicality)))
# technolgical relatedness
map <- readRDS(file = 'maps/jaccard/allyear/techmap_2012.RDa')

skewness(as.vector(map))
# knowledge base diversity
skewness(na.omit(ApplnStats$back_cit_div))
kurtosis(na.omit(ApplnStats$back_cit_div))
#impact
skewness(na.omit(ApplnStats$fwd_cits5_xy))
